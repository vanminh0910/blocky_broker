...
module.exports.authenticate = function(client, username, password, callback) {
  debug('aaaaaaaaaaaaaaaaaaaaaaaaaaa');
  debug(password);
  if (!password) {
    callback(null, false);
    return;
...
module.exports.authorizePublish = function(client, topic, payload, callback) {
  debug('AUTHORIZING PUBLISH', client.authKey == topic.split('/')[1]);
  debug('PAYLOAD', payload.toString());
  debug('TOPIC', topic);
  callback(null, client.authKey == topic.split('/')[1]);
}

module.exports.authorizeSubscribe = function(client, topic, callback) {
  debug('AUTHORIZING SUBSCRIBE', client.authKey == topic.split('/')[1]);
  debug('TOPIC', topic);
  callback(null, client.authKey == topic.split('/')[1]);
}
